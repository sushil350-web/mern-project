...
const PostCard = ({ type, value }) => {
  const postType = type || value.type || "post"; // fix: derive postType reliably
  ...
  useEffect(() => {
    setIsLike(value.likes.includes(user._id)); // fix: proper like check
      ...
      <div className="bg-white p-8 rounded-lg shadow-md max-w-md w-full">
        <div className="flex items-center space-x-2">
          ...
          {value.owner._id === user._id && (
            <div className="text-gray-500 cursor-pointer ml-auto">
              <button
          ...
          {showInput ? (
            <>
              ...
              <button
                ...
                className="text-sm bg-blue-500 text-white px-1 py-1 rounded-md ml-2"
                disabled={captionLoading}
              >
                {captionLoading ? <LoadingAnimation /> : "Update Caption"}
              </button>
              <button
                className="text-sm bg-red-500 text-white px-1 py-1 rounded-md ml-2"
        ...
        <div className="mb-4">
          {postType === "post" ? (
            <img
              ...
              className="object-cover rounded-md max-w-full max-h-[600px]"
            />
          ) : (
            <video
              src={value.post.url}
        ...
        <div className="flex items-center justify-between text-gray-500">
          <div className="flex items-center space-x-2">
            <span
              onClick={likeHandler}
              className="text-red-500 text-2xl cursor-pointer select-none"
              role="button"
        ...
        {show && (
          <form onSubmit={addCommentHandler} className="flex gap-3 mt-2">
            <input
              type="text"
              className="custom-input"
